        ...
        # end of 2 ppl 
            ...
            if(l[:3] == 'Mr.'):
                ...
                if name in plist:
                    ...
                    if husband in plist:
                        print(husband, "   vgghj")
                        person['des'] = plist[husband]['des']
                    plist[name] =  person
                    
                # if husband in plist:
                #     plist[husband]['money'] += money
                #     plist[husband]['land'] += land 
                    
                # else:
                #     # create Husband profile 
                #     person['land'] = land
                #     person['money'] = money
                #     plist[husband:person]
...
def main():
    ...
    q = "select sum(land) from plist where name = 'Sinesh'"
    q = "select sum(land) from plist where FT = 'Jass'"
    # q = "select sum(land) from plist where name = 'Anuj'"
    # q = "select sum(land) from plist where name = 'Jass'"
    # q = "select sum(land) from plist where name = 'Sinesh'"
    # q = "select sum(land) from plist where name = 'Akshayawat'"
    q = q.split()
    name = q[7][1:-1]
